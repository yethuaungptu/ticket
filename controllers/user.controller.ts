...
const register = async (req: any, res: any) => {
    const data: any = await userService.register(req, res);
...
const login = async (req: any, res: any) => {
    ...
    if (data) baseController.ok(res, 'Account login success');
    else baseController.notFound(res, 'Email not found or password not match');
};
...
const createEvent = async (req: any, res: any) => {
    ...
    if (event) baseController.created(res, 'Event create success', event);
    else baseController.error(res, 'Account creation unsuccess');
};

const eventList = async (req: any, res: any) => {
    const events = await eventService.eventList(req, res);
    if (events.length > 0) baseController.ok(res, 'User event list', events);
...
const eventDetail = async (req: any, res: any) => {
    ...
    else baseController.notFound(res, 'Event data not found');
};

const eventUpdate = async (req: any, res: any) => {
    const event = await eventService.eventUpdate(req, res);
    if (event) baseController.ok(res, 'Event update success');
    else baseController.error(res, 'Event update unsuccess');
};

const eventDelete = async (req: any, res: any) => {
    const event = await eventService.eventDelete(req, res);
    if (event) baseController.ok(res, 'Event delete success');
    else baseController.error(res, 'Event delete unsuccess');
};

const purchaseList = async (req: any, res: any) => {
    const tickets = await ticketService.userPurchaseList(req, res);
    if (tickets.length > 0) baseController.ok(res, 'User ticket purchase list', tickets);
    else baseController.notFound(res, 'No ticket purchase list');
};

const purchaseDetail = async (req: any, res: any) => {
    const ticket = await ticketService.userPurchaseDetail(req, res);
    if (ticket) baseController.ok(res, 'User ticket purchase detail', ticket);
    else baseController.notFound(res, 'No ticket purchase detail');
};
